 ...
 * 说明：多点触摸事件解析器
 */
public class MoreEventParser {
    private static final String TAG = "MoreEventParser";
    ...
    private SparseArray<PointF> mPointers = new SparseArray<>();//手指点位集合
    ...
    private float getMotionEventX(@NonNull MotionEvent ev, int activePointerId) {
    ...
    public void parseEvent(MotionEvent event) {
        for (int i = 0; i < event.getPointerCount(); i++) {
            ...
            mPointers.put(pointerId, new PointF(getMotionEventX(event, pointerId), getMotionEventX(event, pointerId)));
        }

        switch (event.getAction()) {
            case MotionEvent.ACTION_POINTER_DOWN:

                Log.e(TAG, "parseEvent: " + mPointers);
